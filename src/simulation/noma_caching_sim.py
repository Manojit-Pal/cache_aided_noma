...
✅ FIX NOMA-BASE-1: CIC flags corrected — weak_cached now checks strong_file,
   strong_cached checks weak_file (physically correct for interference cancellation)
✅ FIX SIM-2: Removed duplicate function block (copy-paste error)
...
class NOMACachingSimulator:
    ...
    def reset_metrics(self):
        ...
        self.metrics = {
            ...
            # SIC/CIC metrics (NOVEL CONTRIBUTION)
            'sic_attempts': 0,
            'sic_successes': 0,
            'sic_failures': 0,
            'cic_opportunities': 0,
            'cic_enabled_weak': 0,    # Weak user benefits from CIC
            'cic_enabled_strong': 0,  # Strong user benefits from perfect SIC
    ...
    def run_single_episode(self, cache: CacheBase, seed: int,
        ...
        # ✅ BUG FIX #4, #5, #7: Correct ALL parameter names, no 'seed'
        ...
        # ✅ BUG FIX #6: Correct call — users list first, then channel_gains
    ...
    def _simulate_noma_pair(self, weak_user: int, strong_user: int,
        ...
        ✅ FIX NOMA-BASE-1: CIC flags now use PAIRED user's file, not own file.
            - weak_cached  = strong_file in cache  (weak user cancels strong's signal)
            - strong_cached = weak_file in cache   (strong user does perfect SIC)
        """
        ...
        # ====================================================================
        # CIC FLAG CONSTRUCTION (✅ FIX NOMA-BASE-1)
        # ====================================================================
        # CIC physics:
        #   weak_cached=True means the weak user has the STRONG user's file
        #   cached → can reconstruct and cancel the strong user's interference.
        #
        #   strong_cached=True means the strong user has the WEAK user's file
        #   cached → can perfectly reconstruct and cancel it (perfect SIC).
        #
        # The OLD code used cache.is_hit(weak_file) / cache.is_hit(strong_file)
        # which checked each user's OWN file — unrelated to interference cancellation.
        weak_cached   = cache.is_hit(strong_file, update_stats=False)  # ✅ FIXED
        strong_cached = cache.is_hit(weak_file,   update_stats=False)  # ✅ FIXED
        ...
        # ====================================================================
        # SIC/CIC SIMULATION (NOVEL CONTRIBUTION)
        # ====================================================================
        ...
        # weak_cached=True → weak user has strong_file → CIC applied for weak user
        if weak_cached:
        ...
        # strong_cached=True → strong user has weak_file → perfect SIC for strong user
        if strong_cached:
            self.metrics['cic_opportunities'] += 1
            if strong_success:
        ...
        # ====================================================================
        # ✅ BUG FIX #9: Cache Learning — correct method per cache type
        # ====================================================================
    ...
    def _simulate_single_user(self, user_id: int, file_id: int,
        ...
        # ✅ BUG FIX #9: Correct cache type dispatch
        is_dqn_cache = hasattr(cache, 'learning_rate')
    ...
    def _compile_results(self, cache: CacheBase) -> Dict:
        ...
        total_noma_users = total_noma * 2  # Each NOMA pair = 2 users
        ...
        results = {
            ...
            # NOMA performance
            'outage_probability': self.metrics['outages'] / total_noma_users,
            ...
            'cic_opportunity_rate': self.metrics['cic_opportunities'] / total_noma_users,
